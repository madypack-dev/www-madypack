...
class TestPresupuestoEndpoints:
    ...
    def test_post_presupuesto_falla_critica_caso_uso_retorna_confirmacion_emergencia(self, client, monkeypatch):
        from src.lead.aplicacion.casos_uso.crear_lead import CrearLeadDesdePresupuesto

        async def mock_ejecutar(self_obj, request, carrito):
            raise RuntimeError("Critical Mocked Crash")

        monkeypatch.setattr(CrearLeadDesdePresupuesto, "ejecutar", mock_ejecutar)
        ...
        # Debe retornar 200 y cargar confirmación a pesar de la falla crítica
        assert response.status_code == 200
        assert "¡Presupuesto Generado!" in response.text
        assert "COT-ERR-" in response.text

        # Verificar que se generó la entrada en el log de fallback
        import os
        assert os.path.exists("logs/failed_leads.log")
        with open("logs/failed_leads.log", "r", encoding="utf-8") as f:
            lines = f.readlines()
