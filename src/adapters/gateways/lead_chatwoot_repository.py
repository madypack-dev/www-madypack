...
class ChatwootContactRepository(ILeadRepository):
    ...
    async def guardar(self, lead: Lead, inbox_id: int) -> str:
        ...
        response = await self.client.post(
            contact_url, headers=headers, json=contact_payload, timeout=10.0
        )
        response.raise_for_status()

        contact_data = response.json()
        contact_obj = (
            contact_data.get("payload", {}).get("contact", {})
            if "payload" in contact_data
            else contact_data
        )
        ...
        if not contact_id:
            raise ValueError(
                f"No se pudo obtener el contact ID de la respuesta de Chatwoot: {contact_data}"
            )

        # 2. Asociar al Inbox
        inbox_url = f"{self.base_url}/api/v1/accounts/{self.account_id}/contacts/{contact_id}/contact_inboxes"
        inbox_payload = {"inbox_id": inbox_id}
        try:
            inbox_response = await self.client.post(
                inbox_url, headers=headers, json=inbox_payload, timeout=10.0
            )
