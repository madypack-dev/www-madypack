...
TARIFAS: dict[str, ConceptoTarifa] = {
    ...
    "manija_plana": ConceptoTarifa(
        nombre="manija_plana", monto=0.35, moneda=Moneda.ARS, fecha=date.today()
    ),
    "personalizacion": ConceptoTarifa(
        nombre="personalizacion", monto=0.20, moneda=Moneda.ARS, fecha=date.today()
    ),
    "pegado": ConceptoTarifa(nombre="pegado", monto=0.10, moneda=Moneda.ARS, fecha=date.today()),
    "confeccion": ConceptoTarifa(
        nombre="confeccion", monto=0.08, moneda=Moneda.ARS, fecha=date.today()
    ),
    "fotopolimero": ConceptoTarifa(
        nombre="fotopolimero", monto=0.05, moneda=Moneda.ARS, fecha=date.today()
    ),
    "bobina_kg": ConceptoTarifa(
        nombre="bobina_kg", monto=1.0, moneda=Moneda.ARS, fecha=date.today()
    ),
    "corte": ConceptoTarifa(nombre="corte", monto=0.02, moneda=Moneda.ARS, fecha=date.today()),
    "confeccion_cuerdas": ConceptoTarifa(
        nombre="confeccion_cuerdas", monto=0.05, moneda=Moneda.ARS, fecha=date.today()
    ),
    "fijo_matriz": ConceptoTarifa(
        nombre="fijo_matriz", monto=1500.00, moneda=Moneda.ARS, fecha=date.today()
    ),
